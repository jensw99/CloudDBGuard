...
public class OPE_RSS extends OPEScheme {
	 ...
	 */
	public OPE_RSS(KeyStoreManager _ks, DBClient _db, int _pbits, int _cbits) {
		...
		M = (long)Math.pow(2, _pbits - 1); // - 1 because the sign takes one bit as well
		N = (long)Math.pow(2, _cbits - 1); // - 1 to make sure the ciphertexts are still short enough after step 4 
	...
	private void loadDictionary(DBLocation id) {
		...
		if(!id.getIdAsPath().equals(dictID)) {
			...
			if(!file.exists()) { 
				...
				sample(mainDict,  -M,                   -M, M-2,                              r_min, r_max-1);
	 ...
	 * @param target the value that is about to be encrypted
	 */
