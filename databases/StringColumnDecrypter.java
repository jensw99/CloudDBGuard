...
/**
 * Class for decrypting a String column
 *
 * @author Tim  Waage
 */
public class StringColumnDecrypter implements Callable<HashMap<byte[], String>> {
	 ...
	 */
	public StringColumnDecrypter (HashMap<byte[], byte[]> _encryptedValues, Set<byte[]> _rowkeysForDecryption, HashMap<byte[], byte[]> _IVs, ColumnState _column) {
	 ...
	 * Encrypts the column
	 */
	public HashMap<byte[], String> call() throws Exception {
		
		// create new HashMap for decrypted Values
		HashMap<byte[], String> decryptedValues = new HashMap<byte[], String>();
	
		if(column.isRNDoverDETStrippedOff()) // no RND layer over DET layer
			for(byte[] key : rowkeysForDecryption) //for(byte[] key : encryptedValues.keySet()) 
				decryptedValues.put(key, Misc.ByteArrayToCharString(column.getDETScheme().decrypt(encryptedValues.get(key))));
		else { //RND layer still existing
			for(byte[] key : rowkeysForDecryption) 								
				decryptedValues.put(
						key, 
						Misc.ByteArrayToCharString(
